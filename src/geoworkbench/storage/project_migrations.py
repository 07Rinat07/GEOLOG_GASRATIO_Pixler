...
def _migrate_v8_to_v9(payload: ProjectPayload) -> ProjectPayload:
    migrated = deepcopy(payload)
    project = migrated.get("project")
    if not isinstance(project, dict):
        raise ProjectMigrationError("Проект версии 8 не содержит объекта 'project'")
    project.setdefault("custom_formulas", {})
    migrated["format_version"] = 9
    return migrated


def _migrate_v9_to_v10(payload: ProjectPayload) -> ProjectPayload:
    migrated = deepcopy(payload)
    project = migrated.get("project")
    if not isinstance(project, dict):
        raise ProjectMigrationError("Проект версии 9 не содержит объекта 'project'")
    migrated.setdefault("tablet_presets", {})
...
def _migrate_v14_to_v15(payload: ProjectPayload) -> ProjectPayload:
    ...
    project = migrated.get("project")
    if not isinstance(project, dict):
        raise ProjectMigrationError("Проект версии 14 не содержит объекта 'project'")
    wells = project.get("wells", {})
    if not isinstance(wells, dict):
        raise ProjectMigrationError("Проект версии 14 содержит некорректный объект wells")
    for well in wells.values():
        if not isinstance(well, dict):
            raise ProjectMigrationError("Некорректная запись скважины версии 14")
        well.setdefault("interpretations", {})
...
def _migrate_v16_to_v17(payload: ProjectPayload) -> ProjectPayload:
    migrated = deepcopy(payload)
    project = migrated.get("project")
    if not isinstance(project, dict):
        raise ProjectMigrationError("Проект версии 16 не содержит объекта 'project'")
    wells = project.get("wells", {})
    if not isinstance(wells, dict):
        raise ProjectMigrationError("Проект версии 16 содержит некорректный объект wells")
    for well in wells.values():
        if not isinstance(well, dict):
            raise ProjectMigrationError("Некорректная запись скважины версии 16")
        well.setdefault("operational_events", {})
...
def _migrate_v18_to_v19(payload: ProjectPayload) -> ProjectPayload:
    migrated = deepcopy(payload)
    project = migrated.get("project")
    if not isinstance(project, dict):
        raise ProjectMigrationError("Проект версии 18 не содержит объекта 'project'")
    wells = project.get("wells", {})
    if not isinstance(wells, dict):
        raise ProjectMigrationError("Проект версии 18 содержит некорректный объект wells")
    for well in wells.values():
        if not isinstance(well, dict):
            raise ProjectMigrationError("Некорректная запись скважины версии 18")
        well.setdefault("lag_correction_profiles", {})
...
def _migrate_v27_to_v28(payload: ProjectPayload) -> ProjectPayload:
    migrated = deepcopy(payload)
    project = migrated.get("project")
    if not isinstance(project, dict) or not isinstance(project.get("wells", {}), dict):
        raise ProjectMigrationError("Некорректный проект версии 27")
    for well in project.get("wells", {}).values():
        if not isinstance(well, dict):
            raise ProjectMigrationError("Некорректная скважина версии 27")
        well.setdefault("analysis_update_history", [])
...
DEFAULT_PROJECT_MIGRATIONS.register(27, _migrate_v27_to_v28)
