...
def load_source_documents(
    ...
    for dataset_id, raw_reference in manifest.items():
        ...
        candidate = project_path.parent / relative_path
        artifact = candidate.resolve(strict=False)
        if not artifact.is_relative_to(assets):
            raise SourceArtifactError(
                f"Source artifact находится вне каталога проекта: {relative_path}"
            )
        before = _regular_file_stat(candidate, f"Source artifact не найден: {relative_path}")
        ...
        try:
            ...
            raise SourceArtifactError(f"Source artifact не найден: {relative_path}") from exc
        after = _regular_file_stat(candidate, f"Source artifact не найден: {relative_path}")
        if (
            after.st_size != reference["size_bytes"]
            or len(raw_bytes) != reference["size_bytes"]
        ):
            raise SourceArtifactError(f"Размер source artifact не совпадает: {relative_path}")
...
def _verify_existing_artifact(artifact: Path, document: LosslessLasDocument) -> None:
    error_message = f"Существующий source artifact повреждён: {artifact.name}"
    before = _regular_file_stat(artifact, error_message)
    if before.st_size != document.size_bytes:
        raise SourceArtifactError(error_message)
    try:
        existing = artifact.read_bytes()
    except OSError as exc:
        raise SourceArtifactError(error_message) from exc
    after = _regular_file_stat(artifact, error_message)
    if (
        after.st_size != document.size_bytes
        or len(existing) != document.size_bytes
        or sha256(existing).hexdigest() != document.sha256
    ):
