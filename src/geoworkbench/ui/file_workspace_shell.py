...
class FileWorkspaceWidget(_LegacyFileWorkspaceWidget):
    """Modern, discoverable shell over the tested file-workspace services."""
    ...
    def _build_header_and_navigation(self) -> None:
        ...
        subtitle = QLabel(
            "Документы, PDF-операции, архивы, логотипы и расчёты "
        ...
        tips = (
            ...
            "Калькулятор, единицы и вертикальные отметки",
        )
        for index, (title_text, tip) in enumerate(zip(titles, tips, strict=True)):
            self.sections.setTabText(index, title_text)
            self.sections.setTabToolTip(index, tip)
    ...
    def _rebuild_document_page(self) -> None:
        ...
        self._document_actions.extend(
            [self.save_document_button, self.save_as_document_button]
        )
        ...
        command_layout.addWidget(
            self._text_button("По ширине", self._fit_width, command_bar)
        )
        command_layout.addWidget(
            self._text_button("Вся страница", self._fit_page, command_bar)
        )
        ...
        hint = QLabel(
            "Выберите страницу и выделите область мышью. Затем примените "
        ...
        self._page_list.setSelectionMode(
    ...
    def _improve_engineering_page(self) -> None:
        ...
        hint = QLabel(
            "Результаты обновляются сразу. После смены категории или единиц "
        ...
        calculator_layout = (
            calculator_group.layout() if calculator_group is not None else None
        )
        if isinstance(calculator_layout, QHBoxLayout):
            calculator_layout.addWidget(self._calculator_error)
        ...
        converter_layout = (
            converter_group.layout() if converter_group is not None else None
        )
        ...
        self.converter_category.currentIndexChanged.connect(
            self._schedule_conversion
        )
        self.converter_source.currentIndexChanged.connect(
            self._schedule_conversion
        )
        self.converter_target.currentIndexChanged.connect(
            self._schedule_conversion
        )
    ...
    def _improve_logo_page(self) -> None:
        ...
        for widget in (
            ...
            if text_signal is not None:
                text_signal.connect(self._schedule_logo_preview)
                continue
            value_signal = getattr(widget, "valueChanged", None)
            if value_signal is not None:
                value_signal.connect(self._schedule_logo_preview)
                continue
            toggle_signal = getattr(widget, "toggled", None)
            if toggle_signal is not None:
                toggle_signal.connect(self._schedule_logo_preview)
    ...
    def _improve_archive_page(self) -> None:
        ...
        for capability in self.archive_service.capabilities():
            ...
            if capability.can_create:
                states.append("создание")
            if capability.can_extract:
                states.append("распаковка")
            if not states:
                states.append("не установлен backend")
            capabilities.append(
                f"<b>{capability.archive_format.value.upper()}</b>: "
                f"{', '.join(states)}"
            )
        self.archive_capabilities.setText(
            " &nbsp; · &nbsp; ".join(capabilities)
        )
    ...
    def _improve_pdf_tools_page(self) -> None:
        ...
        self.pdf_tools_log.setPlaceholderText(
            "Здесь появится журнал: исходный файл, созданный результат "
    ...
    def _apply_theme(self) -> None:
        ...
        self.setStyleSheet(
            ...
            QLabel#hint, QLabel#capabilityStrip {{
            ...
            QFrame#commandBar, QFrame#contextBar, QFrame#floatingCard {{
                background: {colors['panel']};
                border: 1px solid {colors['border']};
                border-radius: 9px;
            }}
            ...
            QTabWidget#fileWorkspaceSections::pane {{
                border: 1px solid {colors['border']};
                border-radius: 9px;
                background: {colors['window']};
            }}
            QTabWidget#fileWorkspaceSections QTabBar::tab {{
                min-width: 150px;
                min-height: 36px;
                padding: 7px 12px;
            ...
            QTabWidget#fileWorkspaceSections QTabBar::tab:hover {{
                background: {colors['hover']};
                color: {colors['text']};
            }}
            QTabWidget#fileWorkspaceSections QTabBar::tab:selected {{
                background: {colors['card']};
                border-color: {colors['border']};
                color: {colors['accent']};
                font-weight: 700;
            }}
            QListWidget#pageList, QTextEdit#operationLog, QTreeWidget,
    ...
    def _sync_document_state(self) -> None:
        ...
        self.undo_document_button.setEnabled(
            is_open and self.document_service.can_undo
        )
        self.redo_document_button.setEnabled(
            is_open and self.document_service.can_redo
        )
        ...
        self._document_info.setText(
            f"<b>{path.name if path else 'Документ'}</b><br>"
            f"{kind} · страниц: {self.document_service.page_count}"
        )
        if self._page_list.count() != self.document_service.page_count:
            self._populate_page_list()
        if self._page_list.count():
            self._syncing_pages = True
            self._page_list.setCurrentRow(
                self.document_service.page_index
            )
            self._syncing_pages = False
    ...
    def _selection_changed(self, selection: QRect) -> None:
        if (
            selection.isNull()
            or selection.width() < 2
            or selection.height() < 2
        ):
            self._selection_info.setText("Область не выбрана")
            return
        ...
        self._selection_info.setText(
            f"{width:.1f} × {height:.1f} pt\n"
            f"Экран: {selection.width()} × {selection.height()} px"
        )

    def _zoom_out(self) -> None:
        self.zoom_spin.setValue(
            max(self.zoom_spin.minimum(), self.zoom_spin.value() - 10)
        )

    def _zoom_in(self) -> None:
        self.zoom_spin.setValue(
            min(self.zoom_spin.maximum(), self.zoom_spin.value() + 10)
        )
    ...
    def _fit_document(self, *, use_height: bool) -> None:
        ...
        width_scale = max(
            0.1,
            (viewport.width() - 34) / max(1, rendered.width),
        )
        scale = width_scale
        if use_height:
            height_scale = max(
                0.1,
                (viewport.height() - 34) / max(1, rendered.height),
            )
            scale = min(width_scale, height_scale)
        self.zoom_spin.setValue(
            max(10, min(800, round(scale * 100)))
        )
    ...
    def _calculate_expression_live(self, *_args: object) -> None:
        ...
        try:
            ...
            return
        self.expression_result.setText(
            format_engineering_value(result)
        )
        self._calculator_error.clear()
    ...
    def _convert_units_live(self, *_args: object) -> None:
        ...
        try:
            ...
            result = self.converter.convert(
                numeric, category, source, target
            )
        except (
            EngineeringExpressionError,
            KeyError,
        ...
        self._converter_equation.setText(
            f"{format_engineering_value(numeric)} "
            f"{self.converter_source.currentText()} = "
            f"{formatted} {self.converter_target.currentText()}"
        )
        self._converter_error.clear()
