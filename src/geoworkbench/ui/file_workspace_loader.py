...
def _is_optional_dependency_error(error: ModuleNotFoundError) -> bool:
    name = error.name or ""
    return name == "fitz" or name == "PIL" or name.startswith("PIL.")
    ...
    def _text(self, key: str) -> str:
        texts = {
            "ru": {
                ...
                "message": (
                    "Основное приложение продолжает работать. Для вкладки «Файлы» отсутствует "
                    "модуль {module}. Выполните команду в активном виртуальном окружении:"
                ),
            ...
            "kk": {
                ...
                "message": (
                    "Негізгі қолданба жұмысын жалғастырады. «Файлдар» қойындысына {module} "
                    "модулі жетіспейді. Белсенді виртуалды ортада пәрменді орындаңыз:"
                ),
            ...
            "en": {
                ...
                "message": (
                    "The main application remains available. The Files tab is missing module "
                    "{module}. Run this command in the active virtual environment:"
                ),
                "copy": "Copy command",
