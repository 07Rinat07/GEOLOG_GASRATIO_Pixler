...
def _result(
    frames: int, *, peak: float, rows: int, records: int,
    baseline: float = 90.0,
) -> PreviewMemoryResult:
    return PreviewMemoryResult(
        frames=frames, buffer_frames=100, baseline_rss_mib=baseline, peak_rss_mib=peak,
        workload_peak_rss_mib=peak,
...
def test_gate_rejects_missing_rows_or_records() -> None:
    violations = evaluate_results(
        _result(100, peak=130.0, rows=50, records=99),
        _result(1000, peak=155.0, rows=99, records=100),
    )
    assert any("lost retained rows" in violation for violation in violations)
    assert any("lost retained records" in violation for violation in violations)


def test_gate_compares_workload_growth_after_different_startup_baselines() -> None:
    violations = evaluate_results(
        _result(100, peak=250.0, rows=100, records=100, baseline=250.0),
        _result(1000, peak=240.0, rows=200, records=200, baseline=130.0),
    )
    assert any("peak RSS growth" in violation for violation in violations)
