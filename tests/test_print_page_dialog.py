...
def test_print_page_settings_map_to_qt_page_layout() -> None:
    settings = PrintPageSettings(PrintPageFormat.A3, PrintOrientation.LANDSCAPE)
...
def test_print_page_dialog_round_trips_settings(qapp) -> None:
    initial = PrintPageSettings(PrintPageFormat.CUSTOM, PrintOrientation.LANDSCAPE, 420.0, 900.0)
...
def test_custom_print_page_maps_millimeter_dimensions() -> None:
    settings = PrintPageSettings(PrintPageFormat.CUSTOM, PrintOrientation.PORTRAIT, 300.0, 1200.0)
...
def test_roll_page_length_follows_content_aspect_and_is_bounded() -> None:
    settings = PrintPageSettings(PrintPageFormat.ROLL, PrintOrientation.LANDSCAPE, 300.0, 297.0)

    regular = settings.page_size_for_content(1000, 3000).size(QPageSize.Unit.Millimeter)
...
def test_roll_dialog_enables_width_but_not_manual_height(qapp) -> None:
    dialog = PrintPageDialog(initial=PrintPageSettings(page_format=PrintPageFormat.ROLL))
