...
Текущая тестовая версия: **0.7.47**. Исправлен импорт GeoScape/Paradox DB со смешанным порядком глубины, пакетный DB → LAS использует явный DEPT/DEPTH/MD, а диапазон каждой кривой редактируется и сохраняется непосредственно в шапке рабочей формы.
