...
Текущая тестовая версия: **0.7.59**. Исправлена критическая регрессия переключения
плотных форм: каждый `TabletTrackWidget` теперь получает активный локализатор до построения
шапки, поэтому колонка с внутренней прокруткой больше не падает с ошибкой `_localizer`.
Форма сначала полностью строится и только затем фиксируется; при ошибке сохраняется последняя
рабочая компоновка. Project format, form schema и tablet layout не изменялись.
