...
Текущая тестовая версия: **0.7.52**. Очистка планшета стала идемпотентной и пропускает уже
удалённые Qt-объекты, поэтому импорт и переключение форм не блокируются ошибкой
`CurveHeaderEditor already deleted`. Шапки кривых уплотнены: пределы, единица, тип шкалы и
инженерная линейка занимают существенно меньше высоты.
