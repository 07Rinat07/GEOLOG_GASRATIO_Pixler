...
Текущая тестовая версия: **0.7.55**. Параметры каждой графической колонки теперь
жёстко прижаты к верхнему краю общего синхронизированного пояса шапки; свободное место
остаётся только под последним параметром, поэтому глубинные сетки сохраняют выравнивание без
растянутых промежутков между шкалами. Исправлен также сбой открытия конструктора из-за
посторонней переменной lag/depth workflow. Пакет журналов создаётся через меню «Справка».
