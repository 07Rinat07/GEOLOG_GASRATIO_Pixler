...
- выбор русского, казахского или английского языка при первом запуске, сохранение настройки
  и общий словарь интерфейса/печатных форм;
- главное окно на PySide6;
- единая команда «Импортировать данные...» для LAS, CSV/TXT и Excel с сохранением
  специализированных сценариев настройки каждого формата;
...
- импорт XLS/XLSX/XLSM через встроенную команду с выбором листа, строки заголовка и индекса,
  составным DATE+TIME mapping, безопасной конвертацией XLS и пересчётом формул через
  LibreOffice Calc;
...
- нормализованный метан `C1_NORM` и отдельная reference-нормализация C1–C5/изомеров с
  кривыми компонентов и `TG_NORM`, ручными эталонными условиями и паспортами источников;
...
- безопасный редактор пользовательских формул с сохранением в проекте, автоматическим
  определением входных LAS/расчётных кривых и ручным принудительным пересчётом;
...
- менеджер интерпретационных интервалов по структуре `Project → Well → Interpretation → Intervals` с типами, подписями, цветами, комментариями, Undo/Redo и экспортом JSON/CSV/Excel.
- отдельная планшетная дорожка активной интерпретации с полосами по типам, hit-testing, синхронным выделением в менеджере и редактируемой панелью свойств.
...
- диапазонные операции LAS Editor: константа, шум min/max с seed, копирование/вставка и
  Undo/Redo; экспорт выбранных глубин/параметров в TXT, CSV и Excel.
