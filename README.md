...
Текущая тестовая версия: **0.7.56**. В библиотеке и редакторе формы появился постоянный
контроль ширины относительно книжного и альбомного A4: программа показывает фактическую
ширину, ожидаемый процент уменьшения и рекомендации — уменьшить колонки, перейти в альбомную
ориентацию, скрыть второстепенные треки или разделить форму. На планшете состояние A4 видно в
строке состояния. Панель статистики интервала теперь автоматически закрепляется справа либо
снизу и не выходит за границы рабочего экрана. Пакет журналов создаётся через меню «Справка».
