...
Текущая тестовая версия: **0.7.58**. Исправлена компоновка плотных шапок:
внутренний viewport теперь всегда содержит только целые строки параметров, а дополнительные
строки доступны через заметную построчную прокрутку. Последний параметр больше не должен
обрезаться границей графика. Экранное отображение кривых переведено на сдержанный профиль:
цвета приглушаются только на экране, обычные тонкие линии становятся легче, а слишком плотные
промежуточные линии сетки автоматически скрываются. Сохранённые цвета, печать и экспорт не
изменяются. Панель статистики интервала остаётся плавающей внутри рабочего экрана.
