...
Текущая тестовая версия: **0.7.51**. Приложение постоянно записывает runtime-диагностику,
а редактирование кривой карандашом обновляет только затронутые треки без перестроения всей
формы. Пакет журналов для анализа создаётся через меню «Справка».
