...
2. Реализован собственный depth-aware peak sampler: фильтрация по видимой глубине и сохранение
   локальных min/max. Curve View и Tablet View ограничивают экранный набор 5000 точками и заново
   выбирают данные при изменении видимого диапазона глубины. Поэтому зум возвращает исходную
   детализацию, а полные массивы остаются источником курсора, расчётов, печати и экспорта.
