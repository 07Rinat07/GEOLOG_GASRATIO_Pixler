...
- пакет `0.7.93`; project `v27`; form `v17`; tablet layout `v25`;
...
- [ ] **WELL-02 — единое обновление данных скважины** (WFLOW-002; после WELL-01).
  ...
  Геологический append, анализы и конфликты профилей PROJ-09 остаются открытыми.
...
### WELL-02: геологическое дополнение (текущий инкремент)

Реализованы свободные интервалы литологии и состава шлама по явному профилю поставщика,
раздельные ID профилей, уникальные экспортные коды, сохранение ручных данных и общий
откат с числовым обновлением. Project v27 хранит снимок профиля и историю добавлений.
Сквозная проверка и release/security gates этого инкремента пока не завершены.
WELL-02 остаётся открытым: отдельные анализы и перенос пользовательских рисунков — далее.
