...
- [CWLS LAS 2.0](https://www.cwls.org/wp-content/uploads/2017/02/Las2_Update_Jan2017.pdf)
  определяет LAS 2.0 как формат обмена цифровыми каротажными кривыми. Поэтому LAS остаётся
  импортом/экспортом, а не контейнером всей редактируемой шламограммы.
- [LogPlot](https://help.rockware.com/logplot8/WebHelp/data_editor_files.htm) хранит кривые,
  литологические описания и downhole symbols в собственном XML-файле `.ldat8`, экспортирует
  LAS отдельно и делает [резервную копию при сохранении](https://help.rockware.com/logplot8/WebHelp/whats_new.htm).
  Это подтверждает разделение рабочего документа и обменного файла.
- [WellCAD](https://wellcad.com/releases/5.7/updates-and-build-notes/) сохраняет свойства Mud Log
  и layout в borehole document `.WCL`, отдельно импортирует/экспортирует LAS и явно разрешает
  конфликты перекрытия при merge. Для GEOLOG принят тот же принцип документа скважины и
  проверяемого добавления данных.
- [Leapfrog Energy](https://help.seequent.com/Energy/2025.2/en-GB/Content/basics/projects.htm)
  автоматически сохраняет проект, поддерживает Save A Copy и zipped backup, а импортированные
  данные сохраняет в дереве проекта. Эти механизмы приняты как ориентир для close guard,
  автосохранения и переносимой резервной копии.
...
- [ ] **PROJ-01:** унифицировать терминологию «источник → проект → экспорт», показать путь
  созданного проекта после первого импорта и передать действие **«Сохранить LAS»** из
  GeoScape2/GS2-маршрута в общий экспортный контроллер.
- [ ] **PROJ-02:** добавить dirty close guard, автосохранение после материальных операций и
  ротационные резервные копии с проверяемым восстановлением после сбоя.
- [ ] **PROJ-03:** добавить **«Сохранить резервную копию»** и переносимый архив с JSON, `.assets`,
  manifest, SHA-256 и проверкой перед открытием.
- [ ] **PROJ-04:** хранить неизменяемый реестр каждого суточного LAS/GS2 с raw artifact,
  fingerprint, диапазоном, import report и транзакционным откатом отдельного добавления.
