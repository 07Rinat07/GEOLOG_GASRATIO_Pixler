...
реализованы; WELL-02…05 остаются незавершёнными. Обязательные release/security gates сохраняются для каждого инкремента;
...
- [ ] **UI-ADAPT-01:** единый laptop/HiDPI-контракт для top-level окон и рабочих областей:
  ...
  создания/сохранения формы с desktop-only минимума на общий screen-fit контракт. Седьмой
  переводит анализ пробы, описание пород, стратиграфический справочник/редактор и NCT на общий
  screen-fit контракт; длинные стратиграфические формы прокручиваются отдельно от закреплённой
  панели действий. Далее пройти остальные редакторы и настройки с regression-проверками малых
  work-area и Windows HiDPI matrix.
