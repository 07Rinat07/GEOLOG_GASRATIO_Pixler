...
LAS/CSV/Excel/Paradox import jobs, выполнение печати, перепривязка project session и команды
дерева проекта проверяются отдельно от `MainWindow`. Qt-слой сохраняет только диалоги,
выбор пользователя и отображение результата.
