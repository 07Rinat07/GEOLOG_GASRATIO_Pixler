...
- [ ] восстановить контракт маршрутизации аннотаций, включая
  `TabletView._annotation_ancestor` либо его документированную замену;
- [ ] устранить аварийное завершение Qt-процесса в полном pytest;
- [ ] исправить 6 ошибок Ruff;
- [ ] устранить 142 ошибки mypy в 33 файлах, начиная с enum/`str`, `None`, сигнатур Qt и
  несогласованных моделей геометрии;
- [ ] выполнить все 1169 тестов без ошибок и падения процесса;
- [ ] провести Windows/HiDPI/PDF/physical print smoke-test.

Критерий: все команды из `TESTING.md` зелёные; сборка остаётся тестовой до выполнения критерия.

## P0 — уменьшить архитектурный риск

- [ ] выделить из `TabletView` отдельные annotation event router, navigation coordinator,
...
Критерий: пользовательские сценарии не меняются, а основные команды тестируются без создания окна.
