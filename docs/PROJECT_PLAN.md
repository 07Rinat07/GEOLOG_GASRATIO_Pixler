...
Решения и приоритеты обновлены 6 сентября 2026 года. Это единственный канонический план проекта. Завершённые
...
- пакет `0.7.93`; project `v25`; form `v17`; tablet layout `v25`;
...
- [ ] **WELL-02 — единое обновление данных скважины** (WFLOW-002; после WELL-01).
  ...
  Следующий локальный инкремент добавляет Qt-независимый `well_update_plan`: полный подсчёт
  ...
  NaN источника не очищает проект. Анализ ничего не применяет; сокращённый diff не является
  подтверждением correction. Проверки схемы/оси общие со строгим append.
  UI и транзакционное применение трёх режимов, геологический append и конфликты профилей
  PROJ-09 остаются открытыми.
