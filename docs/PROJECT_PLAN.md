...
- печатная шапка выводит все семь C1–nC5 rows; автоматическая пагинация заранее резервирует
  место для повторной нижней шапки, поэтому все страницы имеют одну ширину и инженерный
  масштаб, а график заканчивается до шапки; off-scale samples клипуются через bounded overscan
  без искусственных `NaN`-фрагментов и небезопасных painter coordinates;
