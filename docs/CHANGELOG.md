...
## Unreleased
- Gas context/Background: confirmed `exclude_geological`/`technological_gas` intervals are excluded from robust gas-background learning while their measured source values remain immutable and auditable; draft context does not alter the baseline.
...
- Gas context/Reports: добавлен generic `GasContextEvent`/`GasContextRegistry` для повторяющихся технологических газовых интервалов и project-format v35 persistence на уровне скважины; ручной Total Gas хранится как QC reference и не изменяет source curves.
...
- WITS0: добавлен regression-сценарий на 10 окон preview (200 кадров): после вытеснения и compaction сравниваются SHA-256 числовых рядов live и indexed raw replay. RSS масштабирование проверяется отдельным Windows gate с сохранённым baseline.
...
- План проекта актуализирован по `main` на 21.09.2026: устранены устаревшие статусы
  WELL/PROJ/ARCH и PR #76; приоритет отдан ограничению всей памяти WITS preview, диагностике
  сборки/времени, понятным состояниям монитора и полевой приёмке. Новые задачи имеют зависимости,
  ответственных по роли и критерии завершения; реализация этих задач пока не заявляется.
- WITS0: параметры теперь появляются в bounded LIVE PREVIEW сразу после получения кадров, ещё до
  создания постоянной сессии; после Import Review накопленные кадры переносятся в Dataset.
- WITS0: профиль GeoScape GSWITS переведён на CP1251, поэтому кириллический идентификатор скважины
  больше не помечает каждый принятый кадр ошибкой декодирования.
- UI: устранено дублирование верхнего меню WITS; его единственным владельцем остаётся navigation
  controller.
...
- UI-ADAPT-01: состав шлама, библиотека Masterlog, пакетный Paradox-конвертер и Import Review
  переведены на общий work-area контракт; длинные вторичные панели действий прокручиваются, а
  основные кнопки запуска/подтверждения/закрытия остаются видимыми на laptop/HiDPI.
- UI-ADAPT-01: анализ пробы, описание пород, стратиграфический справочник/редактор и NCT
  переведены на общий screen-fit контракт; длинные стратиграфические формы получили отдельную
  прокрутку при закреплённых нижних кнопках, чтобы действия оставались доступны на laptop/HiDPI.
- WELL-02: диалог поздних анализов теперь разрешает применение только после явного выбора
  хотя бы одной fill-строки; конфликтные строки и пустой выбор не закрывают окно как успешный
  no-op.
...
- WELL-06: команда «Подготовить комплект документов» подключена к MainWindow через тонкий UI-
  ...
  зависимости окна от snapshot/exporter/runtime слоёв.
- WELL-06: MainWindow получил отдельную команду повтора неудачных документов; она активируется
  только после partial run и через command-controller повторяет failed outputs той же snapshot-
  ревизии, перезаписывая тот же manifest без нового capture проекта.
