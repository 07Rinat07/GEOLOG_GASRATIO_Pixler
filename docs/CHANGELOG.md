...
- Добавлен отдельный, не изменяющий стандартный расчет, вариант отчёта ОПУС с
  ...
  попадают в XLSX/DOCX-доказательства интервалов и не превращаются в ложный
  однозначный класс. Неподтвержденный пятый показатель, нормализация по TotalGas
  и голосование из Integration Kit сохранены только как vendor reference.
