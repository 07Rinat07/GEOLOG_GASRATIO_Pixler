...
- Каталог планшетных форм сокращён до шести недублирующих рабочих вариантов для глубины и
  времени. Для каждого профиля добавлены согласованные A4-шапки в книжной и альбомной
  ориентации, автоматическое переключение пары при печати и опциональный повтор заголовков
  колонок внизу листа.

- Во вкладке «Отчёты по интерпретации» добавлена «Разгонка газовой смеси»: временная
  диаграмма C1–C5, оценка и вычитание фона, состав пробы, Wh/Bh/Ch, Pixler, явная
  предварительная категория газа/газоконденсата/жидких или тяжёлых УВ, PDF/печать с
  графиком и отдельный компактный режим без графика.

- Исправлена читаемость вкладки «Отчёты по интерпретации» при тёмной системной теме: рабочая область, поля, кнопки и HTML-таблицы получили явную контрастную светлую палитру.

- Добавлена вкладка «Отчёты по интерпретации»: единый безопасный расчёт Gas Ratio/Haworth/Pixler, `C1_NORM`, `DEXP/DEXPC`, отдельные планшетные дорожки, поиск относительных газовых аномалий по всей скважине, предварительная интерпретация «вероятный газ/вероятные жидкие УВ/смешанный тип» и экспорт кандидатов/ручных интервалов в XLSX, DOCX, PDF и на принтер. Проведена ревизия legacy Excel/Word-методик; ошибочные формулы с пропущенными C4/C5 и старые hardcoded-классификации не перенесены.

- Исправлен запуск при устаревшем `.venv`: отсутствие PyMuPDF/`fitz` больше не блокирует всё приложение; вкладка «Файлы» остаётся видимой и показывает команду обновления зависимостей. README и инструкции RU/KK/EN синхронизированы.

- Добавлена независимая вкладка «Файлы»: просмотр и редактирование PDF/изображений, объединение и разделение PDF, экспорт текста в DOCX, конструктор логотипов, безопасные ZIP/TAR и опциональные 7Z/RAR backend, инженерный калькулятор, конвертер дробных единиц и расчёт отметок datum/GL/wellhead/DF/RT/KB-RKB.
...
- Редакторы шапок и планшетных дорожек стали адаптивными: тулбары с overflow-меню, сохраняемые разделители панелей, живой WYSIWYG-контроль свойств элемента и дорожки; импорт SKF получил явные режимы полного комплекта, только формы и только шапки.
...
- Добавлены раздельные каталоги печатных шапок и логотипов: готовые заводские шапки Masterlog, суточной, технологической и аварийной диаграмм, пользовательские CRUD-операции, корректный header-only импорт GeoSketch SKF, вставка логотипов из каталога и выбор любой шапки для любой формы в A4/A3 portrait/landscape; project format обновлён до v22.
