...
- Стандартный расчёт интерпретации теперь создаёт видимые `C1_NORM`, reference-нормализованные
  C1–C5 и `TG_NORM` по явно показанным ROP/BIT/FLOW/E. Для разреженных тяжёлых компонентов
  доля C2–C5 считается интегрально по интервалу, выводится с пятью знаками после запятой,
  а колонка «Отсчётов» переименована в «Точек выше порога».
  Готовые нормализованные кривые, поступившие из LAS/GS2 или серверной программы,
  распознаются по распространённым именам, используются формами и отчётом и защищены
  от перезаписи; в интерфейсе добавлены подсказки по фактическим и эталонным ROP/BIT/FLOW/E.

- Интерпретация кандидатных интервалов использует версионированную палетку Wh/Bh/Ch
  Haworth/DATALOG и диапазоны Pixler, показывает возможное влияние воды только как
  отдельный признак профиля. Встроены две независимые шкалы ЛБА — группа битумоида
  и интенсивность свечения; перекрывающиеся по глубине газ и ЛБА сопоставляются как
  согласованные, частично согласованные, расходящиеся или смешанные признаки без
  изменения исходного заключения геолога.
