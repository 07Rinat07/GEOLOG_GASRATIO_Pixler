...
- Исправлен сбой `NoneType.role` при открытии нового GS2-набора поверх формы,
  сохранившей идентификатор оси предыдущего dataset.
- Применение формы и откат транзакции теперь используют ось уже привязанной
  формы, а не старого layout.
- Устаревший или несовместимый `vertical_index_id` автоматически заменяется
  активной осью текущего набора.
- Для абсолютного времени GeoScape2 сохранена миграция TIME → DATETIME; явно
  выбранная глубина не изменяется.
...
- Added row-based internal scrolling so the last parameter is never partially hidden by the plot body.
- Replaced saturated fallback hue-wheel colours with a restrained professional palette.
- Applied screen-only colour muting and thinner ordinary pens in multi-curve tracks; persisted and printed styles remain unchanged.
- Reduced minor-grid alpha and suppressed unreadable minor lines in narrow tracks.
- Removed obsolete duplicate UI import-controller modules.
- Kept project format v20, form schema v6 and tablet layout v16 unchanged.
