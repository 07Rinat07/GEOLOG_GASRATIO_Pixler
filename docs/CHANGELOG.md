...
- Исправлена печать плотных шапок и последней страницы: место под повторную нижнюю шапку
  резервируется до разбиения диапазона, поэтому последняя форма больше не сжимается по ширине,
  сохраняет общий инженерный масштаб и при необходимости получает дополнительную страницу.
  Все семь строк C1–nC5 входят в обе шапки, а график не продолжается под нижней. Проверка
  выполнена на реальном LAS, GS2 time, BLData, старой DB и synthetic seven-component regression.
