# Статус проекта

Срез: 23 июля 2026 года. Версия пакета: 0.7.28, тестовая сборка.

## Решение о выпуске

Автоматическая часть релизного gate: **зелёная**. Сборка остаётся тестовой до обязательного
ручного Windows/HiDPI/PDF/physical-print smoke-test.

## Подтверждённая рабочая основа

- LAS 1.2/2.0, CSV/TXT/Excel и GeoScape/Paradox workflows;
- безопасные копии, таблица LAS, операции над кривыми, Undo/Redo и формулы;
- проект с несколькими datasets и индексами, формат проекта 15;
- синхронный многотрековый планшет, формы, интервалы и литология;
- редактируемый Masterlog, header/forms, PDF и Print Center;
- крупные и промежуточные сетки экрана/печати с сохранением в форме;
- синхронное выделение числового интервала `Shift + ЛКМ`, сводка всех видимых параметров и экспорт XLSX/CSV;
- аннотации, project assets и миграции старых проектов;
- синхронные наборы пользовательских документов RU/KK/EN.

## Результаты аудита качества

| Проверка | Фактический результат |
|---|---|
| Полный pytest | 1193 пройдено, 10 пропущено; процесс штатно завершён с кодом 0 |
| Ruff | все проверки пройдены |
| mypy | 0 ошибок в 258 исходных файлах |
| Аннотации | восстановлены hit routing, область набора данных/формы и диагностика canvas-wide overlay |
| Архитектура планшета | annotation event router и navigation coordinator изолированы; order/reuse треков вынесен в `TrackLifecycleCoordinator` |
| Интерфейс планшета | верхние панели прокручиваются и не выталкивают окно за границы узкого экрана |
| Экспорт изображений | устранены Windows-блокировка SVG и несовместимость формата PNG в PySide6 |
| Физическая печать/HiDPI | требует подтверждения на целевой Windows-машине |

## Технический долг с наибольшим риском

- `tablet/tablet_view.py` — около 339 КБ;
- `ui/main_window.py` — около 254 КБ;
- `printing/masterlog_renderer.py` — около 91 КБ;
- mypy восстановлен как фактически проходящий критерий; его нужно закрепить в CI;
- старые документы содержали смешение плана и истории, которое теперь вынесено из
  активного статуса.

## Следующая контрольная точка

Завершить Windows GUI/HiDPI/PDF/physical-print smoke-test и подписать релизную матрицу.
Annotation event router, navigation coordinator и безопасная перестановка треков без
пересоздания уже защищены headless/UI-тестами. Следующий архитектурный срез после ручного
gate — создание/удаление треков, затем grid renderer.

Подробности: [аудит](PRODUCT_AUDIT_2026.md), [план](PROJECT_PLAN.md),
[roadmap](ROADMAP.md), [проверки](TESTING.md).
