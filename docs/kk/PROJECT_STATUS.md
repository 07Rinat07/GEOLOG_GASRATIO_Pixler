...
## Ағымдағы 0.7.93 жинағы
- RU/KK/EN тілін ауыстырғанда планшет стандартты пішін, бөлім, технологиялық параметр және каталогтағы жыныс атауларын бірден аударады; пайдаланушы атаулары мен еркін сипаттамалар өзгермейді.

Негізгі іске қосу командасы — `python -m geoworkbench.app.main`. Түбірлік README және ағымдағы
RU/KK/EN нұсқаулықтары синхрондалды, `docs/TESTING.md` бір қолданыстағы release gate сипаттайды,
ал автоматты тесттер module entry point, құжаттаманың ағымдағы нұсқасы және release notes/build
manifest сілтемелерін тексереді. 0.7.87–0.7.89 түзетулері project, form және tablet форматтарын
өзгертпей сақталды.
