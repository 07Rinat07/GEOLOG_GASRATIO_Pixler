...
изменённой. `DataInspectorDialog` только отображает эти модели и инициирует команду. Отчёт
импорта пока хранится runtime в `ProjectSession`; его персистентный provenance требует
следующей версии схемы и не подмешивается в JSON неявно.
