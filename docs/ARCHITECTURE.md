...
одноразовым профилем и временным XLSX. Qt-диалог отвечает только за лист, строку заголовка и
mapping индекса. VBA не запускается, оригинальная книга не изменяется, временные данные
