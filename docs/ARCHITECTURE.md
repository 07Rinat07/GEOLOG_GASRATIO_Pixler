...
При повторе шапки колонок в конце журнала pagination до rendering рассчитывает отдельную
ёмкость последней страницы. Диапазоны перераспределяются при неизменной плотности
depth/time-to-pixel; renderer не имеет права компенсировать нижнюю шапку горизонтальным
уменьшением всей формы. Для одностраничного документа учитываются одновременно верхняя и нижняя
копии шапки, а если они не помещаются вместе с графиком — строится следующая страница.
