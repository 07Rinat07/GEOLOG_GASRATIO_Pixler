...
`excel_adapter` лениво подключает `openpyxl`, читает XLSX/XLSM в read-only и
data-only режимах и преобразует выбранный лист в тот же проверяемый табличный pipeline, что
CSV/TXT. Qt-диалог отвечает только за лист, строку заголовка и mapping индекса. Формулы не
исполняются, оригинальная книга не изменяется, а отсутствие зависимости даёт управляемую
ошибку с командой установки.
