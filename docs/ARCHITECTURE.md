...
Проект `*.geolog.json` является источником истины для продолжаемой работы со скважиной. В нём
сохраняются нормализованные datasets, формы, настройки планшета, интервалы, шламограмма,
кальциметрия, ЛБА, аннотации, значки и provenance. LAS/GS2/DB являются входными артефактами, а
LAS/PDF/CSV/Excel/DOCX/HTML — отдельными проекциями для обмена и публикации. Экспорт не меняет
путь проекта и не заменяет `Ctrl+S`.

Крупные или lossless-артефакты и импортированные изображения могут располагаться в соседнем
каталоге `<project>.geolog.json.assets`. Поэтому текущая переносимая единица состоит из JSON и
его `.assets`; простой перенос только JSON может оставить проект без части вложений. Сам JSON
пишется через временный файл и атомарную замену. Автосохранение, гарантированный close guard,
ротационные резервные копии и единый архив проекта пока не реализованы и остаются требованиями
следующего инкремента.

Ежедневное наращивание LAS изменяет существующий `Dataset` внутри того же `Well`: новые строки
добавляются только после проверки идентичности скважины, индекса, единиц и политики перекрытия,
а ручные well-level слои остаются на месте. Append history хранит имя источника, SHA-256 и
диапазон добавления. Для составного dataset текущий контроллер не сохраняет полный набор raw
суточных файлов и отдельных import reports; целевая модель должна вести неизменяемый реестр
каждого ежедневного источника с возможностью проверки и отката.
...
ради геометрии диапазона. Текущий project format v22 хранит operational events, acquisition
