...
offset как provenance и предупреждает для naive/mixed timezone. Составные DATE+TIME остаются
доступны через тот же сервис с явными `strptime`-форматами и IANA timezone либо фиксированным
UTC offset. Неоднозначные DST wall-clock значения блокируются до выбора явного offset. UI
mapping этих колонок остаётся частью общего мастера импорта.
...
PARAMETER. Он нормализует мнемоники, проверяет десятичные координаты, синхронизирует WELL с
...
Qt-диалог собирает план, но не содержит правил разбора. Поддержка строковых колонок и
DATE/TIME mapping будет добавлена поверх готового `TimeNormalizationService`.
