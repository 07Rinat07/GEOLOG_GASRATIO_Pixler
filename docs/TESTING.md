...
Для среза 0.7.31 в текущем контейнере выполнены `compileall`, сборка wheel и доступный
headless/regression/source-integrity набор: 714 тестов пройдено, 4 платформенных сценария
пропущено. Полный `pytest -q` запущен, но сборка тестов остановилась на 95 Qt/LAS-зависимых
модулях из-за отсутствующих PySide6, pyqtgraph и lasio. Ruff и mypy в контейнере отсутствуют.
Это не заменяет полный gate. Перед выпуском 0.7.31 команды выше и
