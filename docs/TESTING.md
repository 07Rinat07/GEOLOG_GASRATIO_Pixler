...
- во всех встроенных глубинных формах стратиграфия, литология, шламограмма, кальциметрия,
  ЛБА и глубина уменьшаются на 40%, а кривые и текст не сжимаются;
- перечисленные типы допускают ручную ширину 48–2000 px, остальные дорожки сохраняют минимум
  80 px;
- пользовательская форма schema v6 и сохранённый layout v16 мигрируют один раз в v7/v17;
- повторное кодирование и открытие не уменьшают ширину второй раз;
- защищённый шаблон «MASTERLOG — геолого-геохимическая форма» присутствует в библиотеке
  RU/KK/EN и создаёт независимую редактируемую копию;
- экран, сохранение проекта и печать «Фактический размер» используют одинаковые компактные
  пропорции; Fit/автоподбор может перераспределять ширины только для выбранного листа;
- инструкции `FORM_ENGINE.md`, `FEATURES.md`, `PROJECT_STATUS.md`, `PROJECT_PLAN.md` и release
  notes синхронизированы на русском, казахском и английском;
- корневой README не содержит технических release notes.

Минимальная независимая проверка:

```powershell
python tools/check_documentation.py
python -m pytest -q tests/test_compact_geology_columns.py tests/test_form_engine_models.py tests/test_form_column_layout.py tests/test_tablet_layout.py
```

Фактические результаты сборки 0.7.64 фиксируются в `BUILD_MANIFEST_0.7.64.md`.
Полная проверка мышиного resize, HiDPI, PDF и физической печати выполняется на Windows с
установленными PySide6, pyqtgraph и lasio.
...
- команда доступна на панели F4 и в контекстном меню графика;
- справочник показывает только активные depth symbols, локализованные названия и поиск;
- прозрачный режим использует `symbols/transparent`, исходный — `symbols/originals`;
- выбранные дорожка, параметр и глубина проходят валидацию текущего dataset/layout;
- curve anchor сохраняет мнемонику и выборку значения на заданной глубине;
- project-owned image asset дедуплицируется по SHA-256;
- `symbol_id` и `transparent_background` проходят project round-trip и duplicate;
- левое перетаскивание и восемь resize-маркеров сохраняют одну операцию истории на жест;
- экран, PDF и печать используют один canvas object;
- все новые строки присутствуют в RU/KK/EN;
- Windows/PySide6 проверяет вставку обоих вариантов, drag/resize, Undo/Redo, reopen, PDF и печать при DPI 100/125/150%.

Проверка 0.7.61 в доступном контейнере: **103 focused tests passed**; `compileall` — успешно.
Полный Qt/UI-прогон не выполнен из-за отсутствия PySide6, pyqtgraph и lasio и не заменяется
source-contract тестами.
...
## Экранно-безопасная overlay-панель статистики 0.7.60

Обязательные проверки:

- статистика является дочерним overlay `TabletView`, а не top-level `QDockWidget`;
- геометрия полностью помещается в parent rect при обычном и уменьшенном окне;
- отрицательные и слишком большие координаты ограничиваются safe margin;
- после ручного drag resize главного окна только ограничивает позицию, но не возвращает панель вправо;
- кнопка закрытия и toggle action используют один путь очистки interval analysis;
- смена формы и dataset очищает shading, `dataset_selection` и отчёт до перестроения;
- кнопки Copy/Excel/CSV/Clear помещаются при минимальной ширине панели;
- root README не содержит hotfix-описаний, traceback, test counts или release history;
- Windows/PySide6 выполняет drag/resize/close/form-switch на 100%, 125% и 150% DPI.

Проверка 0.7.60 в контейнере: **19 focused passed**; доступная headless-регрессия — **1094 passed, 4 skipped, 3 deselected**. Qt widget-тесты включены, но требуют установленного PySide6.
...
## Создание формы и проверка названий 0.7.64

Обязательные автоматические проверки:

- `tests/test_form_naming.py` — нормализация пробелов и определение дубликатов без учёта регистра;
- `tests/test_form_create_dialog_source_integrity.py` — подключение нового окна и наличие
  RU/KK/EN-подсказок, списка форм, подробностей и блокировки совпадения;
- `tests/test_form_create_dialog.py` — GUI-сценарий отображения заводских и пользовательских форм,
  блокировка дубликата, выбор оси и возврат очищенного имени;
- `tests/test_form_engine_models.py` — уникальность ID, локализованные названия и встроенный
  пользовательский MASTERLOG-шаблон;
- `tests/test_documentation_sync_0762.py` — синхронность инструкций по созданию и именованию форм.

Windows smoke-test должен открыть окно **«Создание формы»**, проверить поиск по названию и
параметру, просмотр подробностей, блокировку совпадающего имени, создание глубинной и временной
формы, сохранение, закрытие и повторное открытие библиотеки.
