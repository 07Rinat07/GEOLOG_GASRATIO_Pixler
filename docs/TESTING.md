...
Для hotfix 0.7.56 в текущем контейнере выполнены `compileall`, focused
width/layout/source-contract-набор — **13 passed**, и доступная headless-регрессия —
**1069 passed, 4 skipped, 3 deselected**. Реальная dock-компоновка требует Windows/PySide6
проверки.

## Floating interval overlay 0.7.58

Обязательные проверки:

- статистика не участвует в minimum width главного окна;
- overlay может перекрывать планшет, но полностью находится в availableGeometry активного экрана;
- move/resize главного окна повторно ограничивает положение панели;
- X окна и «Очистить» удаляют отчёт, dataset selection и синюю заливку;
- применение другой формы очищает интервал до замены widget tree;
- открытие другого dataset/проекта также очищает старую статистику;
- координаты второго монитора с отрицательным или положительным origin поддерживаются;
- Windows/PySide6 проверяет доступность нижних кнопок при DPI 100/125/150%.

В контейнере выполнены `compileall` и focused geometry/integration contract-набор — **27 passed**.
Визуальный Qt smoke-test здесь недоступен из-за отсутствия PySide6/pyqtgraph.

## A4-контроль формы и адаптивная статистика 0.7.56
