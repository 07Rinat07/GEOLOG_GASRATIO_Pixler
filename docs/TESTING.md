...
Для среза 0.7.29 в текущем контейнере выполнены `compileall` и расширенный регрессионный
набор для import jobs, адаптеров и project-session boundary: 105 тестов пройдено,
3 платформенных теста пропущены; wheel пакета 0.7.29 собран без ошибок. Это не заменяет полный gate. Контейнер не
содержит PySide6, pyqtgraph, lasio, Ruff и mypy; попытки установить их не завершились из-за
недоступного package index. Перед выпуском 0.7.29 команды выше необходимо повторить в
установленном Windows-окружении и зафиксировать фактический результат.
