...
Для среза 0.7.32 в текущем контейнере выполнены `compileall`, сборка wheel и доступный
headless/regression/source-integrity набор: 707 тестов пройдено, 4 платформенных сценария
...
Это не заменяет полный gate. Перед выпуском 0.7.32 команды выше и
