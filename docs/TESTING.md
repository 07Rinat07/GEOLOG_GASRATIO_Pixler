# Проверка версии 0.7.17 — hotfix взаимодействия аннотаций
...
- принадлежность обработчиков оси классу `DepthAnnotationsDialog`, а не кнопке выбора цвета;
- наличие защиты событий аннотаций в обоих фильтрах планшета;
- прямое создание с геометрией блока и заметной линией выноски;
- CRUD/Undo/Redo и сериализация модели аннотаций;
- импорт Paradox и раздельные метаданные стандартного шага GeoScape 0,2 м и фактического шага;
- проверка `BLData.db`: LAS `STEP=0.4`, стандарт источника хранится как `0.2`, без скрытого ресэмплинга.

Расширенный non-GUI regression suite: `123 passed`. Полный GUI-набор добавлен, но в текущем контейнере не запускается из-за отсутствия `PySide6` и `pyqtgraph`.

## Обязательный Windows smoke-test аннотаций

1. Откройте планшет, нажмите `F4` и кнопку **«Выноска»** — редактор должен открыться сразу.
2. Введите текст, измените ширину/высоту на вкладке «Геометрия» и нажмите **«Сохранить»**.
3. Перетащите блок за фон или рамку; точка привязки должна остаться на глубине/времени.
4. Измените размер нижним правым маркером.
5. Откройте объект двойным щелчком, измените текст/цвет/шрифт и сохраните.
6. Откройте контекстное меню существующего объекта правой кнопкой; проверьте редактирование, дублирование, блокировку и удаление.
7. Повторите создание через правую кнопку в точной точке дорожки и на временном планшете.
8. Сохраните проект, переоткройте его и сравните экран, PDF и физическую печать.
