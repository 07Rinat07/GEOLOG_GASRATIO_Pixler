...
| PROJECT-003 | Несохранённые изменения защищены close guard, автосохранением и ротационными резервными копиями | запланировано; текущий пользователь обязан явно нажимать `Ctrl+S` |
| PROJECT-004 | Команда резервной/переносимой копии включает JSON, `.assets`, manifest и контрольные суммы | запланировано; сейчас JSON и соседний `.assets` переносятся вручную как одна единица |
...
| I18N-004 | Актуальные пользовательские инструкции на RU/KK/EN | готова синхронная структура из 47 документов на язык; проверка содержания обязательна при каждом пользовательском изменении |
