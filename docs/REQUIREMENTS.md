...
| CALC-016 | Отдельный практический профиль `OPUS Gasomer` по всей скважине | запланировано по `OPUS-01…08`: `100×Ci/TotalGas`, пять workbook-derived индикаторов, построчная синхронность, LOD, уникальная мода/ничья, локальный адаптивный фон без hard gate `0,1 %`, интервальная поддержка и неизменность CALC-013/CALC-015 |
...
| EXPORT-019 | Отчёт профиля `OPUS Gasomer` с проверяемым решением по каждому интервалу | запланировано: прямой класс, независимый Haworth/Pixler, support %, число валидных строк, пять значений/голосов, LOD/QC, локальный фон/контраст, формулы и workbook provenance должны совпадать в UI, XLSX, DOCX, PDF и печати |
