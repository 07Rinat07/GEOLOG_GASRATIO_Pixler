...
- **ARCH-001:** UI вызывает application-контроллеры; бизнес-правила не живут в Qt. Импорт LAS/CSV/Excel/Paradox, print jobs, session binding и команды дерева workspace уже проходят через отдельные service/controller boundaries; следующим срезом устраняется оставшаяся прямая мутация сериализуемой модели из UI.
