...
baseline/peak RSS, время, число вытесненных кадров, compaction, строки Dataset и записи сессии.
Порог роста peak RSS между 1× и 10× — 96 MiB; это консервативный запас для разных Windows
runner и allocator, а не допустимый размер самого preview. Строки и записи не превышают 2×
размер окна. При смене профиля, Python или платформы сравнивать измерения в одинаковой среде;
peak RSS включает импорт библиотек и не измеряет память на конкретный кадр. Эта synthetic
