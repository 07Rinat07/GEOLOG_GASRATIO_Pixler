# GEOLOG GASRATIO@Pixler 0.7.74 — WITS0 raw-захват и офлайн-инвентарь WITSML 2.x

## WITS0 parser и sequence diagnostics

- Добавлен headless `Wits0Parser` с immutable `Wits0ParsedFrame`/`Wits0ParsedField`.
- Поля типизируются как `float`, `integer`, `text`, `date` и `time`; исходная строка и raw value сохраняются.
- Диагностируются unknown record/item, duplicate field, malformed line/value, пустые значения и NaN/Inf.
- `Wits0SequenceTracker` отдельно контролирует каждый record и отмечает first, contiguous, duplicate, gap и out-of-order.
- Live TCP и replay используют один `Wits0StreamProcessor`; равенство результата подтверждено тестами с разными TCP chunk boundaries.
- Окно захвата получило вкладку разобранных полей, parser counters и sequence anomalies.
- Dataset commit, UOM conversion и Import Review остаются следующим этапом.
...
- WITS0 разбирается в `record/item/value`, но пока не выполняет UOM mapping и не создаёт
  `Dataset` или `AcquisitionSession`.
