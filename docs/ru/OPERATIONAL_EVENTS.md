...
Статус: event contract реализован в 0.7.41 и сохраняется в текущем project format v20.
...
События воспроизводятся через тот же revision/QC controller с тем же fingerprint. Следующий срез —
versioned lag/depth correction без изменения source journal. См. [Acquisition replay](ACQUISITION_REPLAY.md).
