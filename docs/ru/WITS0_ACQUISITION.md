...
## Профиль GeoScape / Halliburton

Кнопка быстрой настройки выставляет режим **TCP-клиент**, адрес `192.168.0.100` и порт
`2041`. Это соответствует выбранному в GeoScape режиму «Входящее TCP-соединение
(TCP сервер)»: GeoScape слушает порт, а GEOLOG инициирует подключение и принимает поток.
Входящий порт на компьютере GEOLOG для этого режима открывать не требуется.

Если фактический IPv4-адрес компьютера GeoScape отличается, оператор меняет поле адреса,
сохраняя режим TCP-клиента и заданный в GeoScape порт.
