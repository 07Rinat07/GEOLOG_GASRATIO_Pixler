...
## Текущая сборка 0.7.93
- При переключении RU/KK/EN планшет динамически переводит стандартные названия форм, разделов, технологических параметров и каталожных пород; пользовательские подписи и свободные описания не изменяются.

Каноническая команда запуска — `python -m geoworkbench.app.main`. Корневой README и текущие
RU/KK/EN-руководства синхронизированы, `docs/TESTING.md` описывает один действующий release
gate, а автоматические тесты проверяют module entry point, текущую версию документации и ссылки
на release notes/build manifest. Исправления 0.7.87–0.7.89 сохранены без изменения форматов
проекта, формы и планшета.
