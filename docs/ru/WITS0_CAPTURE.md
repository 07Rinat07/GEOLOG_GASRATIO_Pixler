...
## Операторский быстрый сценарий

Целевой сценарий окна WITS: **выбрать форму → выбрать способ подключения → запустить захват →
смотреть live-параметры → при необходимости подтвердить Import Review и начать постоянную запись**.
Оператор не должен сначала разбираться в служебных полях.

Для типового GeoScape/GSWITS используется готовый preset. Рядом с режимом, IP/interface, портом,
CIDR, raw-каталогом, порогами свободного места и retention должны быть контекстные подсказки.
Отдельная вкладка **Помощь и инструкция** объясняет:

- когда выбирать TCP server и когда TCP client;
- чем IP/interface отличается от адреса удалённого источника;
- зачем нужен CIDR allowlist и почему 0.0.0.0 требует отдельного разрешения;
- зачем raw сохраняется независимо от parser/Import Review;
- чем LIVE PREVIEW отличается от AcquisitionSession;
- что **Пауза просмотра** не останавливает приём;
- как выбрать, изменить и сохранить операторскую форму;
- как открыть полноэкранный WITS monitor и вернуться обратно;
- что означают Gas Ratio/Pixler/DEXP, background/formation/connection/trip gas и alarms.

Если монитор пуст, интерфейс должен показывать конкретное состояние: нет TCP, TCP без байтов,
байты без пригодных кадров, нет оси/каналов, stale data, preview или recording.
...
3. Для другого компьютера укажите конкретный non-global IPv4 доверенного интерфейса и разрешённые
   сети источников в CIDR, например `192.168.10.0/24`. Глобальные сети и `0.0.0.0/0` запрещены.
4. Для `0.0.0.0` дополнительно включите **Разрешить явный bind 0.0.0.0**.
5. Укажите порт и выберите каталог raw-данных.
6. Нажмите **Запустить захват** и подтвердите предупреждение безопасности. Listener принимает только
   peers из CIDR allowlist; остальные соединения закрываются и журналируются как
   `connection_rejected`.
7. В GSWITS сохраните настройки и проверьте состояние соединения.

WITS0 не имеет встроенных шифрования и аутентификации. Никогда не публикуйте listener в интернет
и не настраивайте router port forwarding. Подтверждение требуется при каждом non-loopback запуске
TCP-сервера и не заменяет firewall.
...
Автоматический retention включается только при действительном path-bound marker
`.geoworkbench-wits0-owned.json` в выбранном корневом каталоге. Новый пустой каталог помечается
автоматически. Непустой каталог без marker требует явного принятия оператором; при отказе capture
остаётся доступным, но retention не удаляет файлы. Повреждённый или скопированный marker блокирует
удаление. Посторонние файлы никогда не являются целями retention.
...
- WITS0-порт нельзя открывать в интернет; non-loopback server bind требует явного подтверждения,
  non-global CIDR allowlist для peers и firewall;
