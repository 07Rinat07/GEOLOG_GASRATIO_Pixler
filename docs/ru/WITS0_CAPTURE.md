...
байты без изменения и пропускает каждый полный пакет через типизированный parser. В версии 0.7.76
после подтверждённого Import Review можно запустить append-only `AcquisitionSession`; raw-слой и
parser остаются неизменяемой границей, а Dataset изменяется только через `AcquisitionController`.
...
- current values и специализированные live time/depth graphs будут подключены на следующем этапе;
- reconnect пока не создаёт отдельные connection-gap records, а новая сессия для уже использованной скважины требует отдельной политики;
