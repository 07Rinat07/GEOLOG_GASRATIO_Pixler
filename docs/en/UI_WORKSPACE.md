...
## Command rows outside the native toolbar area — 0.7.72

The main row and the **Form editing (F4)** row are now ordinary responsive widgets inside the central workspace. They no longer participate in the native `QMainWindow` toolbar layout that could raise the window minimum width after clicks, F4, or DPI changes on Windows. Row width comes only from the current central area, and the main-window minimum size is additionally capped to the active screen work area. **Form editing** stays on the right; other actions move to icons and the **`⋯`** menu when space is limited.
...
The main toolbar and the F4 toolbar each use one constrained responsive row. The top rows no longer inherit `QToolBar`, so the application has no native extension arrow that could appear after clicks or after moving the window to an external monitor and push **Form editing** beyond the right edge. The pinned right-side control remains inside the window; other commands switch from labels to icons and then move into the application-owned **`⋯`** menu. Recalculation runs after window, DPI, system-font, style and monitor work-area changes.
