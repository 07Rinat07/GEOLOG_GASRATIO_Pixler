...
Status: implemented in 0.7.42. Acquisition schema: v1. Project format: v18.
...
Project format v18 stores sessions in `well.acquisition_sessions`. Migration `v17 → v18` adds an
empty collection without changing existing project data. The next layer is versioned lag/depth
correction that leaves the append-only source unchanged.
