...
## Раздельные каталоги печатных шапок и логотипов

Печатная шапка отделена от заголовков колонок и выбирается в Print and Export Center независимо
от рабочей формы. Каталог содержит заводские шапки Masterlog, суточного технологического контроля,
технологических исследований, аварийной диаграммы и компактной печати, а также пользовательские
копии. Записи можно добавлять, импортировать из `.skf`, переименовывать, редактировать,
дублировать, удалять и сохранять из текущего редактора. Header-only SKF import вычисляет абсолютные
координаты вложенных Delphi-компонентов и не переносит в шапку вертикальные поля времени/глубины
или тело графика.

Отдельный каталог логотипов хранит пользовательские имена, категории и примечания поверх SHA-256
image assets. Он поддерживает добавление, редактирование, замену изображения, дублирование,
удаление и вставку в текущую шапку. Заводские записи read-only; редактирование начинается с
независимой пользовательской копии. Подробная инструкция: `PRINT_HEADER_AND_LOGO_CATALOGS.md`.
Project format v22 добавляет `logo_catalog`.
